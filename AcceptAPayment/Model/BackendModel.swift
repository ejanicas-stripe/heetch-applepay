...
//  Modified by Eduardo Janicas on 7/20/21
...
class BackendModel : ObservableObject {
	...
	func preparePaymentIntent(paymentMethodType: String, currency: String) {
		...
		let task = URLSession.shared.dataTask(with: request, completionHandler: { [weak self] (data, response, error) in
				...
				// MARK: Create the PaymentIntent. The PaymentIntent contains a client secret, a key that’s unique to the individual PaymentIntent.
	...
	func onCompletion(status: STPPaymentHandlerActionStatus, pi: STPPaymentIntent?, error: NSError?) {
		...
		// MARK: Demo cleanup
        if status == .succeeded || status == .canceled || status == .failed {
